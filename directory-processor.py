...
def should_ignore_path(path: str, ignore_patterns: List[str]) -> bool:
    ...
    Patterns are treated as directory or file names to match exactly.
    """
...
def process_directory(
    ...
    if should_ignore_path(path, params['ignore_patterns']):
        ...
        output_file.write(f"{'  ' * current_depth}[Directory ignored: {os.path.basename(path)}]\n")
        return
    ...
    try:
        ...
        output_file.write(f"{'  ' * current_depth}[Permission denied accessing directory]\n")
        return
        ...
        if os.path.isdir(item_path):
            ...
            if item.endswith('.ipynb'):
                try:
                    ...
                    output_file.write(f"{'  ' * current_depth}├── {item} [Notebook Conversion Error]\n")
                    continue
...
def main():
    parser = argparse.ArgumentParser(description="Process a local directory and create a structured output")
    parser.add_argument("directory_path", help="Path to the directory to process")
    parser.add_argument("--token-limit", type=int, default=10_000, help="Token limit for non-code text files")
    ...
    parser.add_argument("--ignore-patterns", nargs='+', default=['.git', 'node_modules', '__pycache__', '.pytest_cache', 'assets', 'tabs', 'tests'], help="Directories or file patterns to ignore")
    ...
    parser.add_argument("--output", default="directory_structure.txt", help="Output file name")
    parser.add_argument("--split-threshold", type=int, default=1000000, help="Token threshold for splitting output files")
    parser.add_argument("--log-file", default="directory_processing.log", help="Log file name")

...
from .utils.preprocess import process_uploaded_file, create_verses_from_data
...
class ProjectDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get documents with verse statistics
        documents = self.object.documents.annotate(
            total_verses=Count("verses"),
            new_verses=Count("verses", filter=Q(verses__status="NEW")),
            in_progress_verses=Count("verses", filter=Q(verses__status="IN_PROGRESS")),
            completed_verses=Count("verses", filter=Q(verses__status="COMPLETED")),
            reviewed_verses=Count("verses", filter=Q(verses__status="REVIEWED")),
        )
